...
$(document).ready(function(){
    ...
    $(window).trigger('resize');
     ...
     var mySwiper = new Swiper('#podcast .swiper-container', {
      ...
      on: {
          slideChange: function(){
            $(player01)[0].contentWindow.postMessage('{"event":"command","func":"' + 'stopVideo' + '","args":""}', '*');
              $(player02)[0].contentWindow.postMessage('{"event":"command","func":"' + 'stopVideo' + '","args":""}', '*');
              $(player03)[0].contentWindow.postMessage('{"event":"command","func":"' + 'stopVideo' + '","args":""}', '*');
    ...
    $(playBtn01).click(function(){
        $(this).fadeOut('fast');
        $(player01)[0].contentWindow.postMessage('{"event":"command","func":"' + 'playVideo' + '","args":""}', '*');
    });
    $(playBtn02).click(function(){
        $(this).fadeOut('fast');
        $(player02)[0].contentWindow.postMessage('{"event":"command","func":"' + 'playVideo' + '","args":""}', '*');
    });
    $(playBtn03).click(function(){
        $(this).fadeOut('fast');
        $(player03)[0].contentWindow.postMessage('{"event":"command","func":"' + 'playVideo' + '","args":""}', '*');
    });
...
$(document).ready(function(){
    ...
    for(var i in mobile_keys){
        if(navigator.userAgent.match(mobile_keys[i]) != null){ 
            location.href = 'http://mbookand.dothome.co.kr/';
            break;
        }
    }
});
